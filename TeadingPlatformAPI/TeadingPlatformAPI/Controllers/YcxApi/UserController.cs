...
namespace TeadingPlatformAPI.Controllers
{
    public class UserController : ApiController
    {
        ...
        [HttpPost]
        public UnitedReturn UpdateUser(string id,object data,string UserPhoto)
        {
            UserInfo um = new UserInfo();
            ...
            string sql = $"update UserInfo set UserName='{um.UserName}',UserPhoto='{UserPhoto}',UserSex={um.UserSex},ShopId={um.ShopId},UserNumder='{um.UserNumder}',UserAge={um.UserAge},UserIDNumber='{um.UserIDNumber}' where UserNumder='{id}'";
            var res = helpx.ExecuteNonQuery(sql);
            UnitedReturn united = new UnitedReturn();
            if (res > 0)
            {
